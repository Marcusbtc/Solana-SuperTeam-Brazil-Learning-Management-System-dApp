...
function deriveUsername(walletAddress: string): string {
...
async function allocateUsername(preferredBase: string): Promise<string> {
...
async function findUserIdBySessionToken(
  token?: string,
): Promise<string | null> {
  if (!token) {
    return null;
...
async function resolveOauthUser(params: {
  ...
  linked: boolean;
  linkedBySession: boolean;
}> {
  const linkedBySessionUserId = await findUserIdBySessionToken(
    params.linkSessionToken,
  );
...
export async function authRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/auth/wallet/verify", async (request, reply) => {
    ...
    const message = new TextEncoder().encode(
      buildChallengeMessage(challenge.nonce),
    );
    ...
    try {
      ...
      const statusCode =
        typeof (error as { statusCode?: unknown }).statusCode === "number"
          ...
          : 500;
      const message =
        error instanceof Error ? error.message : "OAuth bootstrap failed";
      return reply.code(statusCode).send({ error: message });
    }
  });

  const linkWalletHandler = async (
    request: { body: unknown; session: { userId: string } },

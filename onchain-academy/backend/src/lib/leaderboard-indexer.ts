...
async function fetchToken2022Holders(
...
async function fetchBaselines(
  ...
  return rows.reduce<BaselineMap>((acc, row) => {
...
async function fetchIdentity(
  prisma: PrismaClient,
  wallets: string[],
): Promise<Record<string, { displayName: string; streak: number }>> {
  if (wallets.length === 0) {
    return {};
  }
  ...
  return links.reduce<Record<string, { displayName: string; streak: number }>>(
    (acc, link) => {
      acc[link.address] = {
...
function timeframeXp(balance: number, baseline: number | undefined): number {
...
export async function rebuildLeaderboardSnapshots(
  ...
  const weeklyReference = new Date(
    dayStart.getTime() - 7 * 24 * 60 * 60 * 1000,
  );
  const monthlyReference = new Date(
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const rows = holders.flatMap((holder) => {
      ...
      const weeklyXp = timeframeXp(
        holder.xpBalance,
        weeklyBaselines[holder.walletAddress],
      );
      const monthlyXp = timeframeXp(
        holder.xpBalance,
        monthlyBaselines[holder.walletAddress],
      );
